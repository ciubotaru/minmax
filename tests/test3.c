...
int main()
{
	...
	unsigned int len = 80;
	char *msg = "Testing minl() and maxl()";
	char *failed = "[FAIL]";
	char *passed = "[PASS]";
	...
	long max =
	    maxl(numbers[0], numbers[1], numbers[2], numbers[3], numbers[4]);
	long max_check = numbers[0];
	for (i = 1; i < limit; i++)
	...
	long min =
	    minl(numbers[0], numbers[1], numbers[2], numbers[3], numbers[4]);
	long min_check = numbers[0];
	for (i = 1; i < limit; i++)
