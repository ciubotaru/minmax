...
/* Testing min_int32() and max_int32() */
...
int main()
{
	...
	unsigned int len = 80;
	char *msg = "Testing min_int32() and max_int32()";
	char *failed = "[FAIL]";
	char *passed = "[PASS]";
	...
	int32_t numbers[limit];
	for (i = 0; i < limit; i++) {
		for (j = 0; j < (sizeof(int32_t) * 8); j++) {
			if (rand() % 2)
				numbers[i] |= (1U << j);
			else
				numbers[i] &= ~(1U << j);
	...
	int32_t max =
	    max_int32(numbers[0], numbers[1], numbers[2], numbers[3],
		      numbers[4]);
	int32_t max_check = numbers[0];
	for (i = 1; i < limit; i++)
	...
	int32_t min =
	    min_int32(numbers[0], numbers[1], numbers[2], numbers[3],
		      numbers[4]);
	int32_t min_check = numbers[0];
	for (i = 1; i < limit; i++)
